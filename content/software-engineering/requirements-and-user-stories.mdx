...
<Callout type="tip" title="數據先於直覺">
與其在會議室爭論「使用者應該想要什麼」，不如看漏斗哪一段流失最嚴重、哪個世代留存掉最快。數據指出的痛點，比猜測更值得做。
</Callout>

### 利害關係人（Stakeholders）分析

需求**不是只來自使用者**。利害關係人是任何「會影響專案、或被專案影響」的角色：出錢的主管、法務／資安、維運團隊、客服、合作廠商、最終使用者。漏掉某個利害關係人，往往等於漏掉一整類需求（例如資安要求「通過滲透測試」、法務要求「符合個資法」），這些常常在上線前才爆出來。
...
<Callout type="warning" title="最常被漏掉的三類">
新手只寫快樂路徑；進階者會補失敗／邊界／權限；但**空狀態、併發重複提交、資料驗證**才是實務上最常出包卻最常被略過的 —— 尤其「重複提交」在金流系統是災難級漏洞。
</Callout>
...
另外常見但不在這八項內的還有**可擴展性（Scalability）、可觀測性（Observability）、法規遵循（Compliance）**。

<Callout type="warning" title="NFR 一定要可量化">
不要寫「系統要快」，要寫「95% 的請求在 200ms 內回應」。不可量化的 NFR 等於沒寫 —— 無法驗收，也無法當架構依據。
</Callout>

### 八大特性的量化指標範例

呼應上面那句「NFR 一定要可量化」—— 同樣這八個面向，要配上「怎麼量」與「範例目標值」才算真正可驗收（目標值依專案情境調整）：

| # | 特性 | 量化指標（怎麼量） | 範例目標值 |
| --- | --- | --- | --- |
| 1 | 功能適切性 | 驗收條件通過率、缺陷密度 | 100% AC 通過；缺陷 < 1 / KLOC |
| 2 | 效能效率 | 回應時間 p95／p99、吞吐量 TPS、資源使用率 | p95 < 200ms；峰值 ≥ 1000 TPS；CPU < 70% |
| 3 | 相容性 | 支援的瀏覽器／OS／API 版本數 | 支援最近 2 版 Chrome／Edge／Safari；向後相容前一版 API |
| 4 | 易用性 | 任務完成率、完成時間、SUS 分數、WCAG 等級 | 新手 3 分鐘內完成下單；SUS > 80；WCAG 2.1 AA |
| 5 | 可靠性 | 可用率 %、MTBF、MTTR、錯誤率 | 99.9%（月停機 < 43 分）；MTTR < 30 分；錯誤率 < 0.1% |
| 6 | 資安 | 高風險漏洞數、加密標準、稽核覆蓋 | 0 個 OWASP Top 10 高風險；TLS 1.2+；密碼 bcrypt |
| 7 | 可維護性 | 測試覆蓋率、圈複雜度、變更前置時間 | 覆蓋率 > 80%；圈複雜度 < 10；改一行到上線 < 1 天 |
| 8 | 可移植性 | 部署環境數、容器化、新環境安裝時間 | 可於 Docker／任一主流雲執行；新環境 30 分內部署 |

<Callout type="tip" title="ISO 25010 給軸，你給刻度">
標準只規定「要量哪些面向」，**具體數字要你依專案情境訂**：銀行系統可用率要 99.99%，內部工具 99% 就夠。逐軸問「這個系統在這面向，可驗收的數字是多少？」，不適用就明確標 N/A。
</Callout>
