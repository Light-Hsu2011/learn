...
- **Shared Kernel（共享核心）**：兩個 BC 共享一小部分模型，任何修改都需要雙方同意。適用於緊密合作的團隊
- **Customer-Supplier（客戶-供應商）**：上游 BC 提供服務，下游 BC 消費服務。上游考慮下游的需求，但上游有最終決定權
- **Conformist（跟隨者）**：下游 BC 完全採用上游的模型，不做任何轉換。適用於下游沒有議價能力的情況
- **Anti-Corruption Layer（防腐層）**：下游 BC 在邊界建立轉換層，把上游的模型翻譯成自己的模型。保護自己不受上游變更影響
- **Open Host Service（開放主機服務）**：上游提供公開的 API 和文件化的協議，讓多個下游 BC 整合
- **Published Language（發布語言）**：用標準格式（如 JSON Schema、Protobuf）定義 BC 間交換的資料格式
...
**Context Mapping = BC 之間的外交關係圖，決定誰跟誰怎麼合作。**
</Callout>
