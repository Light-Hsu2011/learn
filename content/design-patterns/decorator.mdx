...
| 模式 | 關係 |
|------|------|
| **Proxy** | 結構相似但意圖不同 — Decorator 加功能，Proxy 控存取。Decorator 由外部組合，Proxy 通常自己管理被代理物件 |
| **Composite** | 都用遞迴組合，但 Composite 是樹狀（一對多），Decorator 是線性鏈（一對一包裝） |
| **Strategy** | 都能改變物件行為，但 Decorator 從外部「包裝」物件，Strategy 從內部「替換」行為 |
| **Chain of Responsibility** | Decorator 鏈和 CoR 鏈結構相似，但 Decorator 每層都會執行，CoR 只有一個 Handler 處理 |
