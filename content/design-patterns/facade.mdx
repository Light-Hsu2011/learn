...
| 模式 | 關係 |
|------|------|
| **Adapter** | 都是包裝層，但 Facade 簡化複雜的多子系統介面，Adapter 轉換單一不相容介面 |
| **Mediator** | 都是協調者，但 Facade 是單向的（Client → 子系統），Mediator 是雙向的（同事之間互相協調） |
| **Singleton** | Facade 常設計為 Singleton，因為通常只需要一個入口 |
| **Abstract Factory** | Facade 可以用 Abstract Factory 來建立子系統的物件，進一步解耦 |
